...
impl Ord for NodeType {
    fn cmp(&self, other: &Self) -> Ordering {
        other
            .partial_cmp(self)
            .unwrap_or_else(|| panic!("Edge::cmp() - cannot compare nodes with type NodeType::N"))
    }
}
impl PartialOrd for NodeType {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        match other {
            NodeType::N(_) => None,
            NC(_, cost) => {
                let NC(_, sc) = self else {
                    panic!("Edge::partial_cmp() - cannot compare NodeType::NC against NodeType::N")
                };
                Some(cost.cmp(sc))
            }
        }
...
impl Debug for Edge {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.debug_tuple("E").field(&self.0).field(&self.1).finish()
    }
}
...
impl Index<Node> for Tracker {
    ...
    fn index(&self, index: Node) -> &Self::Output {
        self.list
            .get(&index)
            .unwrap_or_else(|| panic!("Error: cannot find {index} in tracker {:?}", &self))
    }
}
impl IndexMut<Node> for Tracker {
    fn index_mut(&mut self, index: Node) -> &mut Self::Output {
        self.list
            .get_mut(&index)
            .unwrap_or_else(|| panic!("Error: cannot find {index} in tracker"))
    }
}
...
impl Graph {
    pub fn get_tracker(&self, visited: NodeState, dist: Cost, parent: Option<Node>) -> Tracker {
        Tracker {
            list: self.nodes.iter().fold(HashMap::new(), |mut out, &node| {
                out.entry(node).or_insert(NodeTrack {
                    visited,
                    dist,
                    parent,
                });
...
impl Graph {
    ...
    pub fn import_edges(list: &[Vec<Node>]) -> Result<Self, Error> {
        ...
        list.iter()
            ...
            .for_each(|(src, dst)| {
                ...
                dst.iter().for_each(|dst| {
                    graph.edges.entry(*src).or_default().insert((*dst).into());
                })
            });
        Ok(graph)
    }
    pub fn from_edge_list(edge_list: &[(Node, Node, Cost)]) -> Self {
        ...
        for &(source, destination, cost) in edge_list.iter() {
            let destinations = adjacency_list.entry(source).or_insert_with(HashSet::new);
    ...
    pub fn import_text_graph(file: &str, node_pat: char, edge_pat: char) -> Option<Graph> {
        ...
        let mut g = Graph::new();
        let f =
            File::open(Path::new(file)).unwrap_or_else(|e| panic!("Could not open {file}: {e}"));
...
fn part1(filename: &str) -> io::Result<()> {
    ...
    for (key, value) in l.edges.iter() {
        g.edges
            .entry(*key)
            .and_modify(|e| e.retain(|v| !value.contains(v)));
    }
